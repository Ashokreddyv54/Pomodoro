...
def count_down(count):
    ...
    canvas.itemconfig(text_timer, text=f"{count_mins}:{count_sec}")
...
timer_label=Label(text="Timer",fg=GREEN,font=("Arial",50, "italic"),bg=YELLOW)
timer_label.grid(column=1,row=0)

start_button=Button(text="Start",font=(FONT_NAME,20, "bold"),bg=YELLOW,command=start_timer)
start_button.grid(column=0,row=2)

reset_button=Button(text="Reset",font=(FONT_NAME,20, "bold"),bg=YELLOW,command=reset_timer)
reset_button.grid(column=3,row=2)

check_mark=Label(font=(FONT_NAME,20))
